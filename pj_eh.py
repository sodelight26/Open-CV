    ...
    for cnt in contours:
        ...
        if area > 1200:
            ...
            if len(appox) == 4:
                ...
                # cv2.circle(im gcontour, (int(cx), int(cy)), 5, (255, 0, 255), cv2.FILLED)


                if totalcount.count(id) == 0:
                    totalcount.append(id)
                    
                current_frame_car_count = len(totalcount)
                if current_frame_car_count != prev_frame_car_count:
                    mycursor = mydb.cursor()
                    current_date = date.today()
                    current_time = datetime.now().time()
                    sql = "INSERT INTO car_records (license_plate_text, date, time, total_cars) VALUES (%s, %s, %s, %s)"
                    val = (text, current_date, current_time, current_frame_car_count)
